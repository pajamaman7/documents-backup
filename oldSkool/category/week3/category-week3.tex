    ...
    \item Let $G$ be a group.
        \begin{enumerate}
            \item What interesting functors are there (in either direction) between $\mathrm{Set}$ and the category $[G, \mathrm{Set} ]$ of left $G$-sets? Which of those functors are adjoint to which?
                ...
                %Then take $L:\mathrm{Set}\to [G,\mathrm{Set}]$, which takes a set $X$ in $\mathrm{Set}$ and maps it to an action $G\times X$, defined by $h(g,x)=(hg,x)$. TODO what does this do on morphisms etc?
                Define $L:\mathrm{Set}\to [G,\mathrm{Set}], $ with $L(X)=G\times X$, and for $f:X\to Y,$ take $Lf(g,x)=(g,f(x))$



                No take $R:\mathrm{Set}\to [G,\mathrm{Set}]$ defined by $R(X)=\mathrm{Map}(G,X)=\{f:G\to X\} $. We equip $\mathrm{Map}(G,X)$ with an action, $g\phi(h)=\phi(gh)$. For a morphism $g$ in $\mathrm{Set}$, we map $R(g(x))=$
