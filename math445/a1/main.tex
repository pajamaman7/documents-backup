...
\item Let $f_n(x)=\left( \cos\left( \frac{2x}{n}\right)  \right)^{n^2}$
  ...
  \begin{enumerate}
    ...
    \item Show that $f_n\xrightarrow[{[0,1]}]{c.u}f$.
        ...
        .\end{align*}
        %TODO u.c of the upper, lower bound.
        For large $n$, we can see that both the upper and lower bound on our sequence converge uniformly to $f(x)=e^{-2x^2}$, so by squeeze theorem, we must also have:
