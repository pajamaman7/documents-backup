...
\item Let $f_n(x)=\left( \cos\left( \frac{2x}{n}\right)  \right)^{n^2}$
  ...
  \begin{enumerate}
    ...
    \item Show that $f_n\xrightarrow[{[0,1]}]{c.u}f$.
        ...
        Therefore $|g(f_n(x))-g(f(x))|<\varepsilon$, and $ g\circ f_n\xrightarrow[{[0,1]}]{c.p}g\circ f$.

        %TODO this is awful to read but at least somewhat complete.

        \paragraph{Solution: } The exponential is continuous, so it must be uniformly continuous on a compact set. Since $-2x^2+\frac{2x^{4}}{3n^2}\xrightarrow[{[0,1]}]{c.u}{-2x^2} $, and $-2x^2- \frac{4x^{4}}{n^2}\xrightarrow[{[0,1]}]{c.u}{-2x^2} $, then by the above lemma both the bounds found for $f_n$ in part (a) must converge uniformly to  $\exp\left( -2x^2 \right) $.

        Then by squeeze theorem:
        \[ f_n\xrightarrow[{[0,1]}]{c.u}e^{-2x^2} .\] 
