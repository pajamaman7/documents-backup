...
Triangle inequality: Let $x,y\in \mathbb{R}^{n},$ and take $r=\max \{\phi(x),\phi(y)\} $. %TODO

Non-negativity: Suppose $x\in \mathbb{R}^{n}$. Then $\phi(-x)=|-1|\phi(x)=\phi(x)$, and:
\begin{align*}
    \phi(x+(-x))&\leq \phi(x)+\phi(-x)&\text{By Triangle Inequality}\\
    \phi(0)&\leq 2\phi(x)\\
    0&\leq\phi(x)
.\end{align*}
...
\begin{enumerate}[label= (\alph*)] 
    \item Show that there exists d > 0 such that $\|x - y\| > d$, $\forall x \in E$ and $\forall y \in F$.  
        \paragraph{Solution: }Take $d=\inf_{x\in E,y\in F}\|x-y\|$. Clearly this is less than any $\|x-y\|$ for $x\in E$, $y\in F$, and it cannot be negative since the norm is positive. So then $d\geq 0$.For contradiction suppose $d=0$. %TODO
