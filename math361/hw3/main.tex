...
\item Show that $A$ is a queen if and only if $A^* A$ is an orthogonal projection.
    ...
    %Deepseek wants clarity on isometry implying A*A=I for x, and more clarity on equality of kernels and ranges.

\item Show that $A$ is a queen if and only if $AA^*$ is an orthogonal projection.
    \paragraph{Solution: }%TODO is it as easy as replacing A with A*? perhaps its enough to justify that A*A o.p iff AA* o.p.
